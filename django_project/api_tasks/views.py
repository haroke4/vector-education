...
from api_base.views_authentication import success_with_text, error_with_text
...
class GetQuizQuestionView(APIView):
    def post(self, request: Request):
        ...
        not_completed_quiz_questions = quiz.questions.exclude(id__in=completed_quiz_questions)
...
class AnswerToQuizQuestionView(APIView):
    ...
    def post(self, request: Request):
        ...
        a = UserQuizQuestion.objects.filter(user_tasks_profile=user_tasks_profile, quiz_question=question).first()
        if a is not None:
...
class GetSurveyQuestion(APIView):
    def get(self, request: Request):
        user_tasks_profile: UserTasksProfile = request.user.user_profile.tasks_profile
        question = SurveyQuestion.objects.exclude(answers__user_tasks_profile=user_tasks_profile).first()
        if question is None:
